...
\example The following details a sample Huffman encoding for letters A-E in a toy alphabet. % (note that the last `1' is the probability; the coding starts on each branch). 
\begin{figure}[h]
\centering
...
\item ``I still don't get it'' (Swapnil Garg)
\item ``Swapnil---remember when, in 8th grade, I made JMO and you didn't?'' (Shaya Zarkesh) \\``I made USAMO.'' (Swapnil Garg)
\end{itemize}
\end{document}