...
\definition[Mutual Information] $$I(X ; Y) = \sum_{x_i} \sum_{y_j} P(x_i, y_j) \log \frac{P(y_j | x_i)}{P(y_i)}$$
...
\subsection{Intuition of $C(Y | X)$, Part 1}
...
\subsection{Channels Review (Picking the Channel from the Source)}
...
\subsection{Revisiting $C(Y|X)$ on the Other Side (Picking the Source from the Channel)}

Now that we've covered the input to the channel, we will now consider the output side of the channel. Define a cost for sending information through the channel (limited bandwith for example). Let's call this cost function $b(x_0, y_0) \geq 0$. Pick an arbitrary cost $\beta$ that you're willing to tolerate. We can't tolerate a cost greater than that $\beta$, so only look at sources where the average cost is less than or equal to that $\beta$. 
 %----------
\newpage
...
\item ``What are we supposed to be doing right now?'' (Swapnil) (x4)
...
\item ``Oxford English Dictionary...there has to be something wrong with that right from the name.''
