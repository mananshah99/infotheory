...
\theorem{If $X \sim \mathcal{U}(0,  a)$, $h(X) = \log{a}$}

\begin{proof}
We have $$h(X) = \int x \log \left(\frac{1}{f(x)} \right) dx$$which can be simplified to $$\int_0^a \frac{1}{a} \log(a) dx = \log(a)$$
\end{proof}

\noindent Note that, given a fixed lower bound and upper bound, no pdf can have a larger entropy than a uniform distribution. In the discrete case, if the number of outcomes is doubled, the entropy $H$ goes up by one bit. Note also that the entropy can be 0 or negative.
\begin{itemize}
\item if $a = 1, h(x) = 0$
...
\noindent It is easier to replace the $\log$ with a $\ln$ in the definition of entropy. Integrating by parts, we obtain $$h(X) = 1 + \ln \frac{1}{\lambda} = \ln \frac{e}{\lambda}$$
