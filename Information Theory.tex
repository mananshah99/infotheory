...
\proof 	Assume that $P(X = k) = \frac{e^{-\lambda_1} \lambda_1^k}{k!}$ and $P(Y = n) = \frac{e^{-\lambda_2} \lambda_2^n}{n!}$\\[2em]
		\begin{align*}
		...
		\end{align*}
\example If you have a continuous random variable $Z$ composed of two variables with pmf $f(x,y)$ such that 
		\begin{equation*}
			f(x,y) = \begin{cases}
		...
		what is $f(x|Y=y_0)$?
		Using Bayes Theorem:
			$$f(x|Y=y_0) = \frac{f(x, y_0)}{f_Y(y_0)}$$
		We define $f_Y$ as the distribution of $Z$ exclusively with respect to $Y$. In otherwords, 
			\begin{align*}
		...
		Substituting this value into our original equation we have that
			\begin{align*}
			f(x|Y=y_0) &= \frac{\frac{12}{5} x (2 - x - y_0)}{\frac{2}{5}(4 - 3 y_0)}\\
