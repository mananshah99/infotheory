...
Imagine with had an RV, $X^{\Delta}$, with a pmf that has bins of arbitrary size, $\Delta x$. If we define $x_i$ to be the event that $X$ is in bin $i$, we could write the entropy of $X^{\Delta}$ as 
...
Let $\Delta x \to 0$:
	$$H(X^{\Delta}) = \int f(x) \log{\frac{1}{f(x)}} dx + \infty$$
	
We define the quantity $\int f(x) \log{\frac{1}{f(x)}} dx$ to be the {\bf differential entropy} of the pmf $f$. The quantity $\infty$ is referred to as the precision factor. From this, we determine that the total entropy of the variable increases as the measurements of the variable become more precise. 

We write the differential entropy as either $h(X)$, $H_{\mathrm{diff}}(X)$, or $h(f)$. 
\subsection{Properties of Differential Entropy}
\theorem{If $Y = X + c$, $h(Y) = h(X)$}
\theorem{If $Y = cX$, $h(Y) = h(X)$ + $ \log{|c|}$}
\theorem{If $X \sim U(0,  a)$, $h(X) = \log{a}$}
%\section*{Quotes}
